...
tools = cv2.resize(tools, (250, 50))
tools = tools.astype("uint8")

ml = 150
max_x, max_y = ml + 250, 50
tool_names = ["Pen", "Rectangle", "Curve", "Circle", "Eraser"]
tool_regions = [(ml + i * 50, 0, ml + (i + 1) * 50, max_y) for i in range(5)]
tool_dimensions = {}
...
while True:
    ...
    frame[0:max_y, ml:max_x] = cv2.addWeighted(tools, 0.7, frame[0:max_y, ml:max_x], 0.3, 0)
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # Check if 2 fingers are up
            if index_tip.y < index_base.y and middle_tip.y < middle_base.y:
                is_two_fingers = True
            else:
                is_two_fingers = False

            # Tool selection only with index finger
    ...
    # Start drawing only with 2 fingers
